...
async def list_isos(per_page: Optional[int], cursor: Optional[str]):
    ...
    """
    request = composer.Request(Consts.URL_ISO_LIST) \
...
async def create_iso(url: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(Consts.URL_ISO_LIST) \
...
async def get_iso(iso_id: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(Consts.URL_ISO_ID.assign("iso-id", iso_id)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def list_public_isos():
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(Consts.URL_ISO_PUBLIC_LIST) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()