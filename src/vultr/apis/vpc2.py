...
async def list_vpc2s(per_page: Optional[int], cursor: Optional[str]):
    ...
    """
    request = composer.Request(const.URL_VPC2_LIST) \
...
async def create_vpc2(data: vpc2.CreateVpc2Data):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_VPC2_LIST) \
...
async def get_vpc2(vpc_id: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_VPC2_ID.assign("vpc-id", vpc_id)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def list_vpc2_nodes(vpc_id: str, per_page: Optional[int], cursor: Optional[str]):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    request = composer.Request(const.URL_VPC2_NODES.assign("vpc-id", vpc_id)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}")
...
async def attach_vpc2_nodes(vpc_id: str, data: vpc2.AttachDetachVpc2NodesData):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_VPC2_ATTACH_NODES.assign("vpc-id", vpc_id)) \
        .set_method(HTTPMethod.POST) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .add_header("Content-Type", "application/json") \
        .set_body(data.to_json()) \
...
async def detach_vpc2_nodes(vpc_id: str, data: vpc2.AttachDetachVpc2NodesData):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_VPC2_DETACH_NODES.assign("vpc-id", vpc_id)) \
        .set_method(HTTPMethod.POST) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .add_header("Content-Type", "application/json") \
        .set_body(data.to_json()) \
