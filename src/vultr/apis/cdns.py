...
async def list_cdn_pull_zones():
    ...
    """
    return await composer.Request(const.URL_CDN_LIST_PULL_ZONES) \
...
async def get_cdn_pull_zone(pullzone_id: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_CDN_PULL_ZONE_ID.assign("pullzone-id", pullzone_id)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def purge_cdn_pull_zone_cache(pullzone_id: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_CDN_PULL_ZONE_PURGE.assign("pullzone-id", pullzone_id)) \
        .set_method(HTTPMethod.POST) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def list_cdn_push_zones():
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_CDN_PUSH_ZONES) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def get_cdn_push_zone(pushzone_id: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_CDN_PUSH_ZONE_ID.assign("pushzone-id", pushzone_id)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def list_cdn_push_zone_files(pushzone_id: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_CDN_PUSH_ZONE_FILES.assign("pushzone-id", pushzone_id)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
...
async def get_cdn_push_zone_file(pushzone_id: str, file_name: str):
    ...
    Returns:
        requests.Response: The response from the API.
    """
    return await composer.Request(const.URL_CDN_PUSH_ZONE_FILE.assign("pushzone-id", pushzone_id).assign("file-name", file_name)) \
        .set_method(HTTPMethod.GET) \
        .add_header("Authorization", f"Bearer {get_key()}") \
        .request()
